     ...
     void AVCapture::CaptureWorker() {
        ...
        while (m_IsGrabbing) {
            ...
            while (true) {
                ...
                if (!swsContext) {
                    swsContext.reset(sws_getCachedContext(swsContext.get(), m_Width, m_Height, swpt,
                        m_Width, m_Height, frameFormat, SWS_FAST_BILINEAR, nullptr, nullptr, nullptr));
                }
                ...
                if (auto status = av_image_copy_to_buffer(image->GetData().data(), size, tmpFrame->data, tmpFrame->linesize,
                    static_cast<AVPixelFormat>(tmpFrame->format), tmpFrame->width, tmpFrame->height, 1); status < 0) {
