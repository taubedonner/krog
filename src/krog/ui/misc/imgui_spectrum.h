...
constexpr ImVec4 ColorConvertU32ToFloat4(ImU32 in) {
  float s = 1.0f / 255.0f;
  return {(float)((in >> IM_COL32_R_SHIFT) & 0xFF) * s, (float)((in >> IM_COL32_G_SHIFT) & 0xFF) * s,
          (float)((in >> IM_COL32_B_SHIFT) & 0xFF) * s, (float)((in >> IM_COL32_A_SHIFT) & 0xFF) * s};
}
...
constexpr struct Light {
  ...
  constexpr std::pair<const char*, ImVec4> operator[](uint32_t idx) const { return Values[idx]; }

  constexpr ImVec4 operator()(uint32_t idx) const { return operator[](idx).second; }
} Light;
...
bool DisablingColoredButton(const char* label, const ImVec4& bgColor, const ImVec4& fgColor, bool disabled = false,
...
void PlotTextColored(const ImVec4& color, const char* text, double x, double y, const ImVec2& pixel_offset = {0, 0},
