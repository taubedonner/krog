...
namespace ImGui {
    ...
    namespace Spectrum {
        ...
        void ShowStyleEditor(bool *p_open) {
            if (ImGui::Begin("Spectrum Color Editor", p_open)) {
                    ...
                    for (int i = 0; i < ImGuiCol_COUNT; i++) {
                        ...
                        if (!output_only_modified || memcmp(&col, &ref->Colors[i], sizeof(ImVec4)) != 0) {
                            std::string preview = (foundPreview == Colors::Darkest.Values.end())
                                                  ? fmt::format(
                                            "colors[ImGuiCol_{}]{:>{}} ImVec4({:.2f}f, {:.2f}f, {:.2f}f, {:.2f}f);\n",
                                            name, "=",
                ...
                for (int i = 0; i < ImGuiCol_COUNT; i++) {
                    ...
                    auto foundPreview = std::find_if(colorPalette->begin(), colorPalette->end(),
                                                     [&](auto pl) { return pl.second == style.Colors[i]; });
        ...
        void Init(float fontSize) {
            ...
            props.ButtonMinSize = {((props.Padding.x) + props.IconSize.x),
                                   ((props.Padding.y * 2.0f) + props.IconSize.y)};
    ...
    bool DisablingColoredButton(const char *label, const ImVec4 &bgColor, const ImVec4 &fgColor, bool disabled,
                                const ImVec2 &size) {
        ImGuiStyle &style = GetStyle();
        auto bgHoveredColor = ImVec4{bgColor.x - 0.1f, bgColor.y - 0.1f, bgColor.z - 0.1f, bgColor.w};
        auto bgActiveColor = ImVec4{bgColor.x - 0.2f, bgColor.y - 0.2f, bgColor.z - 0.2f, bgColor.w};
...
namespace ImPlot {
    ...
    void PlotTextColored(const ImVec4 &color, const char *text, double x, double y, const ImVec2 &pixel_offset,
