#[==[
This file took from Kitware/VTK repo
Source: https://gitlab.kitware.com/vtk/vtk/-/blob/master/CMake/FindFFMPEG.cmake

Changes:

  * Adapted for new versioning system
    ...
    if (FFMPEG_${component}_LIBRARY AND FFMPEG_${component}_INCLUDE_DIR)
        ...
        if (_deps_found)
            ...
            if (EXISTS "${version_major_header_path}")
                ...
                file(STRINGS "${version_major_header_path}" version
                        REGEX "#define  *LIB${component_upper}_VERSION_MAJOR ")
                string(REGEX REPLACE ".*_MAJOR *\([0-9]*\).*" "\\1" major "${version}")
                if (NOT major STREQUAL "")
                    set(external_major_version ${major})
                endif ()
            endif ()
            ...
            if (EXISTS "${version_header_path}")
                ...
                if (NOT external_major_version STREQUAL "")
                    set (major ${external_major_version})
                else ()
                    string(REGEX REPLACE ".*_MAJOR *\([0-9]*\).*" "\\1" major "${version}")
                endif ()
